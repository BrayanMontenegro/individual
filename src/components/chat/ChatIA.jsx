...
const ChatIA = ({ showChatModal, setShowChatModal }) => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      const mensajeObtenidos = snapshot.docs.map((doc) => ({
        id: doc.id,
        ...doc.data(),
      }));
      setMensajes(mensajeObtenidos);
    });
    return () => unsubscribe();
  }, []);
  ...
  const enviarMensaje = async () => {
    ...
    try {
      ...
      try {
        ...
        if (!jsonMatch) throw new Error("No se encontró un JSON válido.");
  ...
  const obtenerRespuestaIA = async (promptUsuario) => {
    ...
    const prompt = `Extrae el nombre y la descripción de categoría en este mensaje: "${promptUsuario}". Si el usuario no provee una descripción, genera una descripción corta basada en el nombre. Asegúrate que el nombre y descripción comiencen con mayúsculas. Devuélvelo en JSON como {"nombre": "...", "descripcion": "..."}.`;
        ...
        <ListGroup style={{ maxHeight: "300px", overflow: "auto" }}>
          {mensajes.map((msg) => (
            <ListGroup.Item
              ...
              className={msg.emisor === "ia" ? "text-start" : "text-end"}
            >
